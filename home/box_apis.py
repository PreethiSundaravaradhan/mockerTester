...
class box_api:
    ...
    def _event_get(self):
        ...
        data = {'created_after': '06/06/2018'}
        response = requests.get(uri, headers=self.header, params=data)
        file_meta = json.loads(response.text)
        print(json.dumps(file_meta, indent=2))
        return
    ...
    def _remove_collab(self, collab_id):
        ...
        response = requests.delete(req_uri)
        print(response.status_code)
        return response.status_code


if __name__ == '__main__':
    token = '8o1hgsRV1iheaIWS4jDrTLkO7nl3TPcH'
    box = box_api(token)
    # folder_id = box._collab_one_file('50227983450')
    data = box._get_collab_by_id('13446345687')
    box._remove_collab('13446345687')
    #box._collab_post()
